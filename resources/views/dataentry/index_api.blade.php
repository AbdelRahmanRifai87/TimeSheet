...
@section('content')
    <div class="max-w-5xl p-6 mx-auto px-10">
                        ...
                        @foreach ($shiftTypes as $shiftType)
                            <tr data-id="{{ $shiftType->id }}" class="hover:bg-gray-50 border">
                                <td class="px-4 py-2 border-b border"><input type="text" name="shift_type_name[]"
                                        ...
                                        value="{{ $shiftType->name }}" required></td>
                                <td class="px-4 py-2 border-b border"><input type="text" name="shift_type_description[]"
                                        class="form-input w-full border border-gray-300 rounded px-3 py-2"
                                        value="{{ $shiftType->description }}"></td>
                                @foreach ($dayTypes as $dayType)
                                    <td class="px-4 py-2 border-b border">
                                        <input type="number" step="0.01"
                                            ...
                                            @if (optional($shiftType->rates->where('day_type_id', $dayType->id)->first())->id) data-rate-id="{{ $shiftType->rates->where('day_type_id', $dayType->id)->first()->id }}" @endif
                                            required>
                                    </td>
                                @endforeach
                                <td class="px-4 py-2 border-b border text-center"><button type="button"
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 invisible hidden"
            id="locationModal">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-auto ">
                <form id="locationForm">
                    ...
                    <div class="px-6 py-4 border-b  flex justify-between items-center">
                        ...
                        <button type="button" class="text-gray-400 hover:text-gray-600 text-2xl leading-none font-bold "
                            id="closeLocationModal">&times;</button>
                    </div>
                    <div class="px-6 py-4">
                        <!-- Your form fields here -->
                        <div class="mb-4">
                            <label for="loc_name" class="block text-sm font-medium mb-1">Name <span
                                    class="text-red-500">*</span></label>
                            <input type="text" class="form-input w-full border border-gray-700 rounded px-3 py-2"
                                id="loc_name" name="name">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-input w-full border border-gray-300 rounded px-3 py-2"
                                id="loc_address" name="address">
                        ...
                        {{-- <div class="mb-4">
                            ...
                            <input type="text" class="form-input w-full border border-gray-300 rounded px-3 py-2"
                                id="loc_state" name="state">
                            <div class="text-red-500 text-xs mt-1" id="loc_state_error"></div>
                        </div> --}}
                        <div class="mb-4">
                            <label for="loc_state" class="block text-sm font-medium mb-1">State</label>
                            <select class="form-select w-full border border-gray-300 rounded px-3 py-2" id="loc_state"
                                name="state">
                    ...
                    <div class="px-6 py-4 border-t  flex justify-end">
                        <button type="submit"
                            class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded border">Save
                            Location</button>
                    </div>
                </form>
            </div>
